...
check_dependencies() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        zenity --question --title="Missing Dependencies" \
               --text="Required packages not found: ${missing[*]}. Install now? (Requires sudo)" \
...
setup_virtual_network() {
    ...
    if virsh -c qemu:///system list >/dev/null 2>&1; then
        ...
        if virsh -c qemu:///system net-info default >/dev/null 2>&1; then
            ...
            if [ "$(virsh -c qemu:///system net-info default | grep 'Active:' | awk '{print $2}')" != "yes" ]; then
                zenity --question --title="Network Not Active" \
                       --text="The 'default' network is not active. Start it now?" \
                       --width="$smaller_width" --height="$smaller_height"
                if [ $? -eq 0 ]; then
                    virsh -c qemu:///system net-start default
                    if [ $? -ne 0 ]; then
                        zenity --error --title="Network Error" \
                               --text="Failed to start default network. Using user session instead." \
                               --width="$smaller_width" --height="$smaller_height"
                        echo "qemu:///session"
                        return
                    fi
...
while true; do
    ...
    # Build virt-install command with full path
    virt_command="/usr/bin/virt-install --connect $connect_uri --name \"$vm_name\" --memory $ram_size --os-variant \"$os_variant\""
    ...
    if [ "$connect_uri" == "qemu:///system" ]; then
        ...
        if [ "$(virsh -c qemu:///system net-info default | grep 'Active:' | awk '{print $2}')" == "yes" ]; then
            ...
            zenity --error --title="Network Error" \
                   --text="Default network is not active. Using user networking instead." \
                   --width="$smaller_width" --height="$smaller_height"
